...
print("Output images are stored in cl_sgd_10_basic_outputs folder & are incremental, you could stop the program and ouputs would be probably there in the folder, howeverm it runs for 400 iterations")
for i in range(0,400):
    ...
    plt.savefig('./cl_sgd_10_basic_outputs/log_erms_train.png',bbox_inches='tight')

    df = pd.DataFrame(log_erms_val)
    ax = df.plot(figsize=(10,15))
    ax.ticklabel_format(useOffset=False)

    plt.savefig('./cl_sgd_10_basic_outputs/log_erms_val.png',bbox_inches='tight')
    plt.close("all")
